...
public class DigitSum {
    ...
    public static long countNumber(int pos, int isTight, int sum, String k, int D, long[][][] dp) {
        if (pos == k.length() + 1) {
            return sum == 0 ? 1 : 0;
        }
        if (dp[pos][isTight][sum] != -1)
            return dp[pos][isTight][sum];
        int limit = isTight == 1 ? (int)(k.charAt(pos - 1) - '0') : 9;
        long ans = 0;
        for (int digit = 0; digit <= 9; digit++) {
            int newTight = (isTight == 1) && (digit == limit) ? 1 : 0;
            ans = (ans + countNumber(pos + 1, newTight, ((sum + digit) % D), k, D, dp)) % mod;
        }
    ...
    public static void main(String[] args) {
        try {
            ...
            fout.print((countNumber(1, 1, 0, K, D, dp) - 1 + mod) % mod);
            fout.close();
        } catch (Exception e) {
            e.printStackTrace();
            return;
